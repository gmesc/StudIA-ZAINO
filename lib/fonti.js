...
const ocrLib = require('./ocr');        // eScansione(): la soglia delle scansioni, in un posto solo
...
function importa(vault, id, sorgenti, quando) {
  ...
  for (const src of (Array.isArray(sorgenti) ? sorgenti : [])) {
    ...
    /* ⚠️ È il documento com'era PRIMA del riconoscimento del testo? Dopo l'OCR
       la copia nel contenitore ha un layer in più e quindi un'ALTRA impronta:
       chi ritrascina l'originale — che è rimasto identico, in casa sua — non
       sta portando un documento nuovo, sta riportando quello che c'è già. Senza
       questo controllo entrerebbe col numero successivo, e le evidenze fatte
       finora resterebbero sul gemello. L'impronta di prima sta nell'indice
       (`ocr.improntaOriginale`), scritta dal riconoscimento. */
    const gemello = imp ? gemelloOcr(vault, id, imp) : null;
    if (gemello && fs.existsSync(path.join(dir, gemello))) {
      scartati.push({ file: path.basename(p), motivo: 'già nello zaino come «' + gemello + '» (testo riconosciuto)' });
      continue;
    }
    ...
    /* Il ritorno si riconosce da DUE impronte: quella del file com'era nel
       contenitore, e — se il testo era stato riconosciuto — quella dell'originale
       prima dell'OCR, che è ciò che l'utente ha ancora in mano e ritrascina. */
    const lapide = imp ? (rimossi(vault, id).filter((r) => r && (r.impronta === imp || r.improntaOriginale === imp))[0] || null) : null;
...
async function elimina(vault, id, nome, opt) {
  ...
  const imp = impronta(p);
  let pagine = 0, improntaOriginale = '';
  try {
    const idx = JSON.parse(fs.readFileSync(percorsoIndice(vault, id, f), 'utf8'));
    pagine = (idx.pages || []).length;
    /* ⚠️ Se il testo era stato RICONOSCIUTO, il file nel contenitore ha un layer
       in più e un'ALTRA impronta rispetto all'originale rimasto in casa
       dell'utente. Quella di prima la sa solo l'indice (`ocr.improntaOriginale`)
       — e l'indice fra due righe se ne va col documento. Senza portarsela nella
       lapide, chi ritrascina l'originale dopo averlo tolto non era «un ritorno»
       ma un documento nuovo col numero dopo, e le evidenze restavano appese al
       vecchio: misurato il 17 agosto 2026 nel laboratorio della guida, dove
       «02 Scheda fotografata» tolta e rimessa è tornata come «03». */
    improntaOriginale = str(idx.ocr && idx.ocr.improntaOriginale);
  ...
  const lapide = { nome: f, impronta: imp, pagine, tolto: str(o.quando) || new Date().toISOString() };
  if (improntaOriginale && improntaOriginale !== imp) lapide.improntaOriginale = improntaOriginale;
  lista.push(lapide);
...
/**
 * Cambia il NOME di un documento, tenendo in piedi tutto ciò che lo nomina.
 *
 * ⚠️ IL NUMERO NON SI TOCCA. `03` non è decorazione: è ciò che scrivono i
 * rimandi (`pdf:03#p=7`) negli appunti, nelle mappe e nei capitoli. Cambiarlo
 * vorrebbe dire rincorrere ogni rimando di ogni file per riscriverne uno solo
 * — e sbagliarne uno significa un link che apre il documento sbagliato senza
 * dire niente. Si cambia la parte leggibile e basta: il legame resta.
 *
 * ⚠️ IL NOME PER ESTESO, invece, è citato in sei posti — è l'elenco che `usi()`
 * legge per dire «a questo documento sono appesi 43 lavori»: il file in
 * `MATERIALI/PDF/`, il suo indice in `Indici-PDF/`, il campo `materiale` di
 * `_evidenze.json` e di `_album.json`, il testo degli appunti e delle mappe, e
 * il `nome` scritto sulle lapidi di `_rimossi.json`. Rinominare toccandone cinque
 * su sei è peggio che non rinominare: il documento c'è, il lavoro sopra no.
 *
 * L'ordine conta: prima il FILE, poi chi lo nomina. Al contrario, un errore a
 * metà lascerebbe dei riferimenti che puntano a un documento inesistente; così
 * il caso peggiore è un documento rinominato con qualche riferimento vecchio,
 * che `usi()` sa ancora trovare col nome di prima — e ciò che non si è potuto
 * riscrivere si dice in `avvisi`, invece di sparire.
 */
function rinomina(vault, id, nome, titoloNuovo) {
  const out = { nome: '', error: '', tocchi: { evidenze: 0, album: 0, appunti: [], mappe: [], lapidi: 0 }, avvisi: [] };
  if (!vault || !idValido(id)) { out.error = 'contenitore non valido'; return out; }
  const vecchio = path.basename(str(nome));
  if (!vecchio || !/\.pdf$/i.test(vecchio)) { out.error = 'non è un documento'; return out; }
  const dir = dirPdf(vault, id);
  const pVecchio = path.join(dir, vecchio);
  try { if (!fs.statSync(pVecchio).isFile()) { out.error = 'non è un file'; return out; } }
  catch (e) { out.error = 'documento non trovato'; return out; }

  /* Il numero di adesso si conserva, la parte leggibile si ripulisce con le
     stesse regole dell'import: chi rinomina non deve poter scrivere un nome che
     l'import non avrebbe mai accettato. */
  const num = (/^(\d{1,3})[\s._-]+/.exec(vecchio) || [])[1] || '';
  const pulito = str(titoloNuovo)
    .replace(/\.pdf$/i, '')
    .replace(/^\d{1,3}[\s._-]+/, '')
    .replace(/[\\/:*?"<>|\x00-\x1f]/g, ' ')
    .replace(/\s+/g, ' ')
    .trim()
    .slice(0, 90);
  if (!pulito) { out.error = 'un documento senza nome non si distinguerebbe dagli altri'; return out; }
  const nuovo = (num ? nn(parseInt(num, 10)) + ' ' : '') + pulito + '.pdf';
  out.nome = nuovo;
  if (nuovo === vecchio) return out;   // niente da fare, e non è un errore

  const pNuovo = path.join(dir, nuovo);
  /* ⚠️ «Esiste già?» non si chiede con `existsSync` e basta: su macOS il volume
     non distingue le maiuscole, quindi correggere «dispensa» in «Dispensa»
     troverebbe sé stesso e si rifiuterebbe. Il criterio è l'inode, ed è lo
     stesso che ha già cancellato una mappa dell'utente quando lo si è ignorato
     (`appunti.stessoFile`). */
  let stesso = false;
  try {
    const a = fs.statSync(pVecchio), b = fs.statSync(pNuovo);
    stesso = (a.ino === b.ino && a.dev === b.dev);
    if (!stesso) { out.error = 'esiste già un documento con questo nome'; return out; }
  } catch (e) { /* il nuovo non c'è: è il caso normale */ }

  try { fs.renameSync(pVecchio, pNuovo); }
  catch (e) { out.error = e.message || 'documento non rinominato'; return out; }
  /* L'indice segue il documento: porta il suo nome, e un indice col nome di
     prima è una lente che trova pagine di un documento che nessuno apre più. */
  try {
    const iVecchio = percorsoIndice(vault, id, vecchio);
    if (fs.existsSync(iVecchio)) fs.renameSync(iVecchio, percorsoIndice(vault, id, nuovo));
  } catch (e) { out.avvisi.push('indice non rinominato: ' + (e.message || e)); }

  const base = corsi.cartella(vault, id);
  const appunti = require('./appunti');

  /* I due JSON: si cambia SOLO il campo `materiale` che nomina questo
     documento, riscrivendo il resto com'era. Non si passa dai loro moduli di
     salvataggio apposta — normalizzano, e una rinomina non è il momento di
     scoprire che una lista bianca ha mangiato un campo nuovo. */
  for (const [chiave, percorso] of [['evidenze', require('./evidenze').percorso(vault, id)],
                                    ['album', require('./album').percorso(vault, id)]]) {
    let doc;
    try { doc = JSON.parse(fs.readFileSync(percorso, 'utf8')); }
    catch (e) { if (e.code !== 'ENOENT') out.avvisi.push(path.basename(percorso) + ' illeggibile: non riscritto'); continue; }
    const voci = Array.isArray(doc) ? doc : (doc.evidenze || doc.voci || []);
    let n = 0;
    for (const v of voci) if (v && v.materiale === vecchio) { v.materiale = nuovo; n++; }
    if (!n) continue;
    try { appunti.writeAtomic(percorso, JSON.stringify(doc, null, 2)); out.tocchi[chiave] = n; }
    catch (e) { out.avvisi.push(path.basename(percorso) + ' non riscritto: ' + (e.message || e)); }
  }

  /* Appunti e mappe: qui il nome sta nel TESTO — nel frontmatter di un appunto,
     dentro un nodo di mappa — e si sostituisce dov'è scritto per esteso. I file
     di servizio (`_indice.md`, `_riga.json`, `_evidenze.json`) cominciano per
     underscore e non si toccano: i due JSON sono già passati di sopra. */
  const re = new RegExp(escapeRe(vecchio), 'g');
  for (const [cart, ext, dove] of [['APPUNTI', /\.md$/i, 'appunti'], ['MAPPE', /\.json$/i, 'mappe']]) {
    let files = [];
    try { files = fs.readdirSync(path.join(base, cart)); }
    catch (e) { if (e.code !== 'ENOENT') out.avvisi.push(cart + '/ illeggibile: non riscritto'); continue; }
    for (const nf of files.sort()) {
      if (!ext.test(nf) || nf.charAt(0) === '_' || nf.charAt(0) === '.') continue;
      const pf = path.join(base, cart, nf);
      let testo;
      try { testo = fs.readFileSync(pf, 'utf8'); } catch (e) { out.avvisi.push(nf + ' illeggibile: non riscritto'); continue; }
      if (testo.indexOf(vecchio) < 0) continue;
      try { appunti.writeAtomic(pf, testo.replace(re, nuovo)); out.tocchi[dove].push(nf); }
      catch (e) { out.avvisi.push(nf + ' non riscritto: ' + (e.message || e)); }
    }
  }

  /* Le lapidi: se questo documento viene tolto domani, la sua traccia deve
     portare il nome di ADESSO — o al ritorno riprenderebbe quello vecchio. */
  const lista = rimossi(vault, id);
  let nl = 0;
  for (const l of lista) if (l && l.nome === vecchio) { l.nome = nuovo; nl++; }
  if (nl) {
    const err = scriviRimossi(vault, id, lista);
    if (err) out.avvisi.push('tracce non riscritte: ' + err);
    else out.tocchi.lapidi = nl;
  }
  return out;
}
...
/** Il documento di questo contenitore che è nato da QUESTO contenuto, se il
 *  riconoscimento del testo l'ha trasformato: il nome del PDF il cui indice
 *  porta `ocr.improntaOriginale` uguale all'impronta data, o `null`. */
function gemelloOcr(vault, id, imp) {
  if (!imp) return null;
  ...
  for (const f of files) {
    ...
    let dati;
    try { dati = JSON.parse(fs.readFileSync(path.join(dir, f), 'utf-8')); } catch (e) { continue; }
    if (dati && dati.ocr && str(dati.ocr.improntaOriginale) === imp) {
      return str(dati.pdf) || f.replace(/\.json$/i, '') + '.pdf';
    }
  }
  return null;
}
 ...
 * conoscerli entrambi.
 */
function scriviIndice(vault, id, nomePdf, pagine, motore, ocr) {
  if (!vault || !idValido(id)) return { error: 'contenitore non valido' };
  ...
  /* ⚠️ Il campo `ocr` — chi ha riconosciuto il testo di una scansione, quando,
     e l'impronta del file com'era PRIMA — si CONSERVA alla riscrittura. La
     reindicizzazione che segue il riconoscimento passa da qui senza sapere
     dell'OCR, e un indice riscritto che dimentica il campo è la lista bianca
     che mangia i campi nuovi: il documento tornerebbe a dirsi «mai riconosciuto»
     e il ritrascinamento dell'originale creerebbe il doppione. */
  let ocrVoce = null;
  if (ocr && typeof ocr === 'object') ocrVoce = ocr;
  else {
    try { ocrVoce = (JSON.parse(fs.readFileSync(percorsoIndice(vault, id, nome), 'utf-8')) || {}).ocr || null; }
    catch (e) { ocrVoce = null; }
  }
  const intero = { pdf: nome, npages: lista.length, motore: str(motore) || 'pdfjs', pages: lista };
  if (ocrVoce) intero.ocr = ocrVoce;
  const corpo = JSON.stringify(intero, null, 1) + '\n';
...
function leggiIndici(vault, id) {
  ...
  for (const f of files.sort((a, b) => a.localeCompare(b, 'it'))) {
    ...
    /* Il verdetto «sembra una scansione» si dà QUI, con la soglia di lib/ocr:
       il renderer che deve mostrare il bottone del riconoscimento non conosce
       numeri — glieli dice l'indice, da un posto solo.
       ⚠️ Un documento GIÀ riconosciuto non è più una scansione per definizione,
       anche se di testo ne è uscito poco (una foto di tre righe resta sotto la
       soglia): senza questa condizione l'app riproporrebbe per sempre un
       riconoscimento già fatto, e «riprova» e «mai fatto» sono due cose diverse. */
    const scan = ocrLib.eScansione(pagine);
    const ocrVoce = (dati && typeof dati.ocr === 'object' && dati.ocr) || null;
    /* ⚠️ «Da riconoscere» = sotto la soglia E mai riconosciuta. La seconda metà
       viene da `ocr.pagine`, la memoria di quali pagine hanno già il layer:
       il conteggio da solo non basta, perché una foto con tre righe resta
       sotto la soglia anche DOPO il riconoscimento — e rileggerla vorrebbe
       dire scriverle addosso un secondo layer, con ogni parola doppia. */
    const fatte = (ocrVoce && Array.isArray(ocrVoce.pagine)) ? ocrVoce.pagine : [];
    const daRiconoscere = scan.daRiconoscere.filter((n) => fatte.indexOf(n) < 0);
    documenti.push({
      pdf: str(dati && dati.pdf) || f.replace(/\.json$/i, '') + '.pdf',
      motore: str(dati && dati.motore),
      scansione: scan.scansione && !ocrVoce,
      pagineVuote: scan.pagineVuote,
      daRiconoscere,
      /* ⚠️ Un riconoscimento FERMATO a metà: il campo `ocr` c'è (le pagine
         fatte sono dentro) ma ne restano di mai lette. Senza questa voce il
         bottone Aa↗ sparirebbe proprio mentre il toast dice «Aa↗ per finire» —
         successo alla prima prova a mano di Giacomo, punto 5. */
      ocrDaFinire: !!ocrVoce && daRiconoscere.length > 0,
      ocr: ocrVoce,
...
module.exports = {
  ...
  importa, percorsoIndice, haIndice, scriviIndice, leggiIndici, gemelloOcr,
  RIMOSSI, impronta, rimossi, elimina, lapideDi, dimentica, usi, rinomina
};
