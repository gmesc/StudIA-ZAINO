 ...
 * SOTTOTITOLI), i nomi cominciano con la sigla del corso invece che con il
...
const mat = require('./materiali');   // dove va ogni tipo di file dentro un progetto
 ...
 *
 * Con `progetto` va DENTRO il progetto, nella cartella del suo tipo:
 * `Progetti/<id>/MATERIALI/{Video,Audio,PDF,Web,Documenti}`. È la condizione
 * perché un progetto si possa passare a qualcuno copiando una cartella sola —
 * e perché la numerazione per progetto abbia senso: se i file atterrano nella
 * radice comune, «il prossimo numero di questo progetto» non vuol dire niente.
 *
 * Senza `progetto` resta la radice del vault, per i vault a corpus unico.
 */
function destinazione(tipo, nomeFile, progetto) {
  if (!progetto) return DESTINAZIONE[tipo] || 'Fonti';
  return path.join('Progetti', progetto, mat.CARTELLA, mat.cartellaPerFile(nomeFile));
}
...
/**
 * Pagine di servizio: esistono nei corsi salvati da internet ma non insegnano
 ...
 * @param {Object} opz   { numeraDa: 1, forzati: [percorsi relativi], progetto } — `forzati`
 *                       sono i file ripescati dagli scarti nell'anteprima; `progetto`
...
function pianifica(files, opz) {
  ...
  const progetto = (opz && opz.progetto) || null;
  ...
  const materiali = candidati.map((c, i) => {
    ...
    return {
      ...
      dir: destinazione(c.tipo, c.nome, progetto),    // un file ripescato a mano finisce fra le fonti
  ...
  for (const t of trascrizioni) {
    ...
    // la trascrizione sta accanto al suo video: dentro il progetto se il video ci sta
    video.abbinati.push({
      ruolo: 'trascrizione-ufficiale', origine: t.rel, abs: t.abs, size: t.size,
      dir: progetto ? path.join('Progetti', progetto, mat.CARTELLA, mat.nomeInProgetto('Trascrizioni'))
                    : 'Trascrizioni',
