...
async function scriviLayer(percorsoPdf, pagine, opz) {
  ...
  for (const pag of (Array.isArray(pagine) ? pagine : [])) {
    ...
    for (const w of pezziDaScrivere(pag)) {
      ...
      daScrivere.push({ testo, x, y, angolo, size, largVista, riga: w.riga });
    }
    ...
    for (const riga of perRiga) {
      const size = Math.max.apply(null, riga.map((pz) => pz.size));
      ...
      for (let k = 0; k < daEmettere.length; k++) {
        ...
        /* ⚠️ Gli spazi in coda: TANTI quanti ne servono a riempire il vuoto
           fino alla parola dopo, non uno per cortesia. pdf.js fonde due run
           nello stesso item solo se il salto del `Td` combacia con
           l'avanzamento disegnato; sui vuoti larghi della dattilografia
           («fra   di   loro») un solo spazio lasciava uno scarto grande,
           pdf.js spezzava l'item e lo spazio pendente moriva con lui —
           misurato: nella stessa pagina «con espressione» era fuso e intero,
           «fra / loro» spezzato e incollato. Il tetto a 8 lascia spezzare i
           salti da colonna, dove uno spazio non è comunque un'informazione. */
        let nSpazi = 1;
        if (k < daEmettere.length - 1 && spazioW > 0) {
          ...
          nSpazi = Math.max(1, Math.min(8, Math.round((dxTesto - avanz) / spazioW)));
        }
        /* per l'ultima parola lo spazio lo mangia il bordo dell'oggetto, e va
           bene: lì la riga finisce */
