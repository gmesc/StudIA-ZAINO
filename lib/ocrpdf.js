...
async function scriviLayer(percorsoPdf, pagine, opz) {
  ...
  for (const pag of (Array.isArray(pagine) ? pagine : [])) {
    ...
    /* Il corpo tipico del TESTO di questa pagina: lo danno le righe che hanno
       lettere. Serve alle righe che non ne hanno — i filetti di trattini — il
       cui inchiostro è alto quattro pixel: un'evidenza alta quanto
       quell'inchiostro è un capello, e Giacomo la scambiava per un guasto. */
    const corpiTesto = perRiga
      .map((riga) => mediana(riga.map((pz) => pz.size)))
      .filter((c) => c >= 8);           // i filetti non votano il corpo di pagina
    const corpoPagina = mediana(corpiTesto);
    for (let iRiga = 0; iRiga < perRiga.length; iRiga++) {
      ...
      let mediaSize = mediana(riga.map((pz) => pz.size));
      /* ⚠️ La riga dall'inchiostro SOTTILE — il filetto di trattini, anche
         quando Tesseract ci legge dentro finte lettere («es = ee Em») — prende
         il corpo del testo di pagina, ridotto: un'evidenza alta tre punti è un
         capello che sembra un guasto (schermo di Giacomo). Il metro è
         l'inchiostro, non l'alfabeto: il testo vero non è mai alto un terzo
         del corpo di pagina. Il tetto della distanza dalla riga sopra, qui
         sotto, le impedisce comunque di invadere. */
      if (corpoPagina > 0 && mediaSize < corpoPagina * 0.45) {
        mediaSize = corpoPagina * 0.6;
      }
      let dSopra = Infinity;
      ...
      for (const [f2, chiave2] of [[font, chiaveElv], [fontMono, chiaveMono]]) {
        ...
        /* ⚠️ La scala di riga vive in una FORBICE, e i due lembi hanno ragioni
           diverse. Il tetto è la MEDIANA: pdf.js spezza l'item quando una
           parola disegnata SCONFINA oltre il salto verso la prossima di più di
           0,2 corpi (`negativeSpaceMax`, letto dal suo sorgente), e con l'item
           muore lo spazio — sopra la mediana metà delle parole sconfina. Il
           pavimento è il quantile 0,2: sotto, le parole nascono così strette
           che gli spazi di coda non bastano più a ricucire. DENTRO la forbice
           comanda l'ULTIMA parola vera della riga: è l'unica che gli spazi non
           possono coprire — pdf.js li scarta in fondo a ogni oggetto — e con
           la scala bassa usciva col bordo mozzo («subingress|o», schermo di
           Giacomo). Le altre parole arrivano comunque in fondo al loro posto,
           riempite dagli spazi. */
        const ordinati = rapporti.slice().sort((a, b) => a - b);
        const q20 = ordinati[Math.floor(ordinati.length * 0.2)];
        const q50 = ordinati[Math.floor(ordinati.length * 0.5)];
        let tzF = q20;
        for (let u = riga.length - 1; u >= 0; u--) {
          if (!/[\p{L}\p{N}]/u.test(riga[u].testo)) continue;
          let wU = 0;
          try { wU = f2.widthOfTextAtSize(riga[u].testo, size); } catch (e) { break; }
          if (wU > 0) tzF = Math.min(Math.max(q20, riga[u].largVista / wU), q50);
          break;
        }
