 ...
 *
 * ⚠️ `horizontalScale` allarga o stringe il testo perché copra ESATTAMENTE il
 * riquadro misurato: Helvetica non ha le metriche del carattere fotografato, e
 * senza questa forzatura la selezione col mouse prenderebbe la parola accanto.
 * Misurato nello spike: con la forzatura lo scarto è 0,00 pt.
 */
async function scriviLayer(percorsoPdf, pagine, opz) {
  ...
  for (const pag of (Array.isArray(pagine) ? pagine : [])) {
    ...
    for (const w of pag.parole || []) {
      ...
      const size = Math.max(PUNTI_MINIMI, alto * 0.95);
      ...
      try {
        p.drawText(testo, {
          x, y, size, font, opacity: 0, rotate: degrees(angolo),
          horizontalScale: wFont > 0 ? Math.max(1, (largVista / wFont) * 100) : 100
        });
        scritte++;
      } catch (e) { saltate++; }
    }
