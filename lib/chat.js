...
function crea(vault, zaino, opts = {}) {
  ...
  return salvaSessione(vault, zaino, { version: 1, id: randomUUID(), title: 'Nuova chat',
...
async function invia(vault, zaino, input = {}, opts = {}) {
  ...
  try {
    ...
    if (!session.messages.length) session.title = user.content.replace(/\s+/g, ' ').slice(0, 72);
...
module.exports = { LIMITI, RUOLI, elenco, leggi, crea, elimina, invia, contesto, wikipedia, systemPrompt, estraePdf };
