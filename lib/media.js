...
/**
 * Toglie un media dal contenitore, mandandolo nel Cestino di chi chiama
 * (`opt.cestina`), come fa `fonti.elimina` con i documenti. Nessuna lapide:
 * i media non hanno un indice da riagganciare — reimportando, il numero si
 * riassegna con la regola di sempre.
 */
async function elimina(vault, id, nome, opt) {
  ...
  if (!f || !tipoDi(f)) return { error: 'non è un media' };
  let p = '';
  for (const t of Object.keys(SOTTO)) {
    const c = path.join(dir(vault, id, t), f);
    try { if (fs.statSync(c).isFile()) { p = c; break; } } catch (e) { /* si guarda nell'altra */ }
  }
  if (!p) return { error: 'media non trovato' };
  try {
    if (typeof o.cestina === 'function') await o.cestina(p);
    else fs.unlinkSync(p);
  } catch (e) { return { error: e.message || 'media non rimosso' }; }
  return { file: f, error: '' };
}

/**
 * Chi si appoggia a questo media: appunti e mappe che lo nominano — per nome o
 * per rimando `video:NN` — e i ritagli dell'album presi dai suoi fotogrammi.
 * Stessa forma e stessa ragione di `fonti.usi`: chi conferma deve sapere che
 * cosa sta staccando, e «non lo so» non è «no». `evidenze` resta 0: le parole
 * chiave vivono nei documenti.
 */
function usi(vault, id, nome) {
  ...
  const f = path.basename(str(nome));
  if (!f) { out.error = 'media non valido'; return out; }
  const base = corsi.cartella(vault, id);
  const num = (/^(\d{2})\s/.exec(f) || [])[1] || '';
  const re = new RegExp(escapeRe(f) + (num ? '|video:' + num + '(?![0-9])' : ''));
  ...
  out.quanti = out.appunti.length + out.mappe.length + out.ritagli;
  return out;
}

function escapeRe(s) { return String(s).replace(/[.*+?^${}()|[\]\\]/g, '\\$&'); }

module.exports = { SOTTO, idValido, tipoDi, dir, elenco, nomi, nomeDestinazione, importa, elimina, usi };
