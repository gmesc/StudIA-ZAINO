...
const appunti = require('./appunti'); // e il nome della cartella delle versioni, che non si esporta
...
function esclusioni(id, opts) {
  ...
  /* ⚠️ Le VERSIONI degli appunti non escono MAI, nemmeno quando gli appunti
     escono: sono una rete di sicurezza di questa macchina — le copie di ciò che
     stava per essere perso — non contenuto da spedire. In un pacchetto per gli
     allievi sarebbero rumore, e per giunta rumore fatto di stesure vecchie che
     l'autore ha riscritto apposta. */
  fuori.push(id + '/APPUNTI/' + appunti.VERSIONI + '/*', id + '/APPUNTI/' + appunti.VERSIONI);
