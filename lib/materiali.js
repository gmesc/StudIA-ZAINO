...
/**
 * Tutto ciò che si trascrive: la lista che serve a chi non distingue fra video
 * e audio (l'elenco dei media di un corso, il drop nel vault, l'import di una
 * cartella, che cosa può entrare in un piano).
 *
 * ⚠️ Esiste perché quelle quattro porte ne tenevano ognuna una COPIA, tutte
 * senza `.ogg .opus .aiff`: un `.opus` — WhatsApp, Telegram, i registratori —
 * entrava nello zaino e si vedeva, ma messo in `Media/` di un corso non veniva
 * elencato, e l'app non diceva niente (invariante 4, misurato il 23 agosto).
 * Chi ha bisogno di questa lista la CHIEDE qui, non la riscrive.
 */
const EXT_MEDIA = EXT_VIDEO.concat(EXT_AUDIO);
...
/**
 * Gli id dei contenitori che hanno una cartella `MATERIALI/`: i corsi e **gli
 * zaini**.
 *
 * ⚠️ Gli zaini stanno in una radice loro (`Zaini/`) ma per i materiali sono
 * corsi in tutto e per tutto: stessa cartella `MATERIALI/PDF`, stessi indici,
 * stessi rimandi `pdf:NN`. Dimenticarli qui vorrebbe dire che un documento
 * dentro uno zaino non si trova — e `srcUrl` ripiegherebbe in silenzio su
 * `<vault>/Fonti/<file>`, cioè aprirebbe niente senza dire perché.
 */
function contenitoriConMateriali(vault) {
  if (!vault) return [];
  const out = [];
  for (const radice of [corsiLib.radice(vault), path.join(vault, corsiLib.RADICE_ZAINI)]) {
    for (const id of dirs(radice)) {
      if (fs.existsSync(path.join(radice, id, CARTELLA))) out.push(id);
    }
  }
  return out;
}
/* Il nome di prima, per chi lo chiama da fuori. */
const corsiConMateriali = contenitoriConMateriali;

/** Vero se il contenitore (corso o zaino) si porta dentro i propri materiali. */
function haMateriali(vault, id) {
  return !!vault && !!id && fs.existsSync(path.join(corsiLib.cartella(vault, id), CARTELLA));
}
...
function cartelle(vault, sub, corso, solo) {
  ...
  /* ⚠️ `corsiLib.cartella()` e non `radice + id`: è la funzione che risolve le
     DUE radici — `Corsi/` e `Zaini/` — ed è il perno su cui poggia la modalità
     zaino. Ricostruire il percorso a mano qui vorrebbe dire che i materiali
     sono l'unica cosa che non lo segue. */
  const dentro = (id) => {
    const base = path.join(corsiLib.cartella(vault, id), CARTELLA);
    for (const n of nomiInCorso(sub)) agg(path.join(base, n));
    agg(path.join(base, sub));
  };
...
module.exports = { SOTTO, NOMI, CARTELLA, ATTESA, EXT_VIDEO, EXT_AUDIO, EXT_WEB, EXT_MEDIA,
  nomeInCorso, nomiInCorso, cartellaPerFile, cartelleCorso, corsiConMateriali, contenitoriConMateriali,
