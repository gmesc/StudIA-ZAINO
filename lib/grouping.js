'use strict';
/**
 * grouping — euristiche di raggruppamento dei materiali in corsi candidati.
 *
 * Serve a due cose:
 ...
 *
 * Idea di fondo: i corsi di solito sono blocchi CONTIGUI di materiali numerati che
 ...
 * tutti i materiali non distinguono niente. Nei corsi registrati è tipico
...
/** Slug ASCII per la cartella del corso. */
function slug(s, max) {
  const t = normalizza(s).replace(/\s+/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '');
  return (t.slice(0, max || 40).replace(/-+$/, '')) || 'corso';
}

/** Trasforma un blocco grezzo nella forma-corso del piano. */
function aCorso(gruppo, ordine) {
...
/** Proposta euristica completa: dal digest ai corsi candidati. */
function proponi(dg, opzioni) {
  ...
  return blocchi(materiali, gran).map(aCorso);
}
...
module.exports = {
  ...
  radiciComuni, titoloGruppo, intervallo, rationale, slug, aCorso, proponi
};
