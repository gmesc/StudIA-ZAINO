 ...
 * confronta: `{ tipo:'pagina'|'tempo'|'foto', valore }`, o `null` se non c'è.
 *
 * ⚠️ Il terzo tipo è nato con l'Album Foto: si può ritagliare anche
 * un'IMMAGINE, e lì il punto non è né una pagina né un secondo — è l'immagine
 * stessa (`da`, l'id della foto). Serviva un tipo suo e non un riuso di
 * `pagina`: il tipo entra nel seme dell'identità, e senza, il ritaglio a
 * pagina 12 di un PDF e quello preso da una foto potrebbero collidere.
 */
function punto(v) {
  ...
  const da = str(v && v.da).trim();
  if (idValido(da)) return { tipo: 'foto', valore: da };
  return null;
}
...
function nomeFile(voce, ext) {
  ...
  const dove = (pt && pt.tipo === 'tempo')
    ...
    /* `__f<id>` per un ritaglio preso da una foto: nel Finder si legge subito da
       quale immagine viene, come `__p007` dice da quale pagina. */
    : (pt && pt.tipo === 'foto') ? '__f' + pt.valore
...
function didascaliaDiRipiego(voce) {
  ...
  if (pt && pt.tipo === 'foto') return stem(voce.materiale) + ' — ritaglio';
...
function normalizzaVoce(v) {
  ...
  if (pt.tipo === 'tempo') base.t = pt.valore;
  else if (pt.tipo === 'foto') base.da = pt.valore;
  else base.pagina = pt.valore;
  return base;
}
...
function perOrdine(a, b) {
  ...
  /* ⚠️ Il valore di un punto «foto» è un id, cioè una STRINGA: sottrarla darebbe
     `NaN`, e un comparatore che torna NaN lascia l'ordine a caso — un album che
     cambia disposizione a ogni riapertura senza che nessuno capisca perché. */
...
function gemello(voci, voce) {
  ...
  for (const x of voci) {
    ...
    if (px.tipo === 'tempo'
      ? Math.abs(px.valore - pv.valore) > TOLLERANZA_T
      : px.valore !== pv.valore) continue;
...
function rimuovi(vaultPath, courseId, id, opt) {
  ...
  const pFile = path.join(dir(vaultPath, courseId), letto.voci[i].file);
  const pMini = letto.voci[i].mini ? path.join(dir(vaultPath, courseId), letto.voci[i].mini) : '';
  const fine = () => {
  ...
  /* ⚠️ `opt.cestina` è iniettata da chi chiama — nell'app è `shell.trashItem`
     di Electron: un ritaglio o una foto sono lavoro dell'utente, e devono poter
     tornare dal Cestino di sistema come i documenti (`fonti.elimina`). Il
     Cestino è asincrono, quindi CON `cestina` la risposta è una Promise; senza,
     resta la cancellazione sincrona di sempre — è ciò che serve alle prove.
  ...
  if (typeof o.cestina === 'function') {
    return Promise.resolve()
      .then(() => { if (fs.existsSync(pFile)) return o.cestina(pFile); })
      .then(() => {
        /* la miniatura con lei — e che non si cestini non è un guasto */
        if (pMini && fs.existsSync(pMini)) return Promise.resolve(o.cestina(pMini)).catch(() => {});
      })
      .then(fine, (e) => ({ tolto: false, usi: u, error: (e && e.message) || 'cancellazione fallita' }));
  }

  try { fs.unlinkSync(pFile); }
  ...
  if (pMini) {
    try { fs.unlinkSync(pMini); } catch (e) { /* passi */ }
  }
