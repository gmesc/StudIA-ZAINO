...
const TIPI = { 'image/png': '.png', 'image/jpeg': '.jpg', 'image/webp': '.webp' };
...
function normalizzaVoce(v) {
  ...
  if (!materiale || !pt || !rect) return null;
  ...
  if (pt.tipo === 'tempo') out.t = pt.valore; else out.pagina = pt.valore;
  return out;
}
...
function elenco(vaultPath, courseId) {
  return leggiIndice(vaultPath, courseId);
}
...
function salva(vaultPath, courseId, ingresso, adesso) {
  ...
  if (!v) return { voce: null, giaCera: false, error: 'il ritaglio non dice da quale documento, da quale pagina o da quale area viene' };
  const d = dir(vaultPath, courseId);
  ...
  const img = daDataUrl(ingresso && ingresso.dati);
...
module.exports = {
  CARTELLA, INDICE, FORMATO, TOLLERANZA, TIPI,
  dir, percorso, percorsoImmagine, nomeFile,
  identita, idValido, normalizzaRect, normalizzaVoce, daDataUrl, misuraPng, firmaOk,
