 ...
 * caricherebbe il CLAUDE.md del progetto e si porterebbe dietro contesto che
 * non c'entra nulla con la scrittura di un capitolo.
 */
...
function transitorio(motivo) {
  return /429|529|5\d\d|rate.?limit|overload|sovracc|timeout|tempo massimo|temporan|try again|connect|ECONN|ETIMEDOUT|socket/i.test(motivo || '');
}
...
async function chiedi(o) {
  ...
  const tentativi = o.tentativi == null ? 3 : Math.max(1, o.tentativi);
  let ultimo = '';
