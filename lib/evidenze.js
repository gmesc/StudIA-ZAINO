...
const CAMPI = ['id', 'exact', 'prefix', 'suffix', 'colore', 'tratto', 'strato',
  'lezioneId', 'capitoloId', 'capitolo', 'materiale', 'pagina', 'creato', 'nota'];

/* LA POSTILLA (`nota`): il perché di una sottolineatura, scritto quando lo si
   sa — «contraddice p. 4», «chiedere al prof».

   ⚠️ Nel modello W3C un'annotazione ha due metà, il BERSAGLIO e il CORPO.
   StudIA aveva solo il bersaglio (`prefix/exact/suffix`, il TextQuoteSelector):
   la ragione per cui si è segnata una frase — l'unica cosa che valeva — si
   perdeva, perché scriverla in un appunto è un cambio di stanza e per una riga
   non lo si fa.

   ⚠️ NON ENTRA NELL'IDENTITÀ. Sta accanto a `colore` e `tratto`: sono commento
   e aspetto, non identità. Scriverla o correggerla non deve far nascere una
   seconda evidenza — è la trappola catalogata («aggiungere un campo al SEME di
   un'identità cambia gli id di ciò che è già nei vault»), e `test/evidenze.js`
   la inchioda con un valore d'oro.

   ⚠️ E SI SCRIVE SOLO QUANDO C'È, in coda al record: un campo assente resta
   assente, non diventa `""`. Gli altri campi si scrivono sempre — anche vuoti —
   e aggiungerne uno allo stesso modo avrebbe cambiato i BYTE di ogni
   `_evidenze.json` già nei vault, al primo salvataggio, per una funzione che
   quell'utente non ha mai usato. */

/** Il tetto della postilla: **una riga**, e il limite si fa rispettare qui.
 *
 * ⚠️ Non è pigrizia. Il giorno che la postilla accetta tre paragrafi, chi
 * studia ha due posti dove scrivere — uno cercabile (gli appunti), uno no — e
 * non saprà mai in quale ha messo quella cosa. Il posto per un pensiero lungo
 * è l'appunto, che la lente trova. */
const NOTA_MAX = 200;

/** Una postilla: una riga sola, senza a capo, entro il tetto. */
function notaValida(t) {
  return str(t).replace(/[\x00-\x1f]+/g, ' ').replace(/\s+/g, ' ').trim().slice(0, NOTA_MAX);
}
...
function normalizzaVoce(v) {
  ...
  /* ⚠️ In coda e SOLO se c'è: vedi la nota su `CAMPI`. L'ordine delle chiavi è
     quello di inserimento, quindi un record senza postilla resta identico byte
     per byte a com'era prima che questo campo esistesse. */
  const nota = notaValida(v.nota);
...
/**
 * Scrive (o cancella) la POSTILLA di un'evidenza — il terzo gemello di
 * `colora` e `tratta`, e per la stessa ragione: il testo evidenziato, il chip
 * nell'elenco e l'appunto che la cita devono leggere lo stesso valore.
 *
 * ⚠️ Una postilla vuota la TOGLIE, invece di scrivere `""`: cancellare un
 * commento è un gesto che deve esistere, e un campo vuoto sul disco sarebbe
 * indistinguibile da una postilla mai scritta.
 *
 * ⚠️ L'id NON cambia, perché la postilla non è nell'identità: chi la scrive
 * ritrova la stessa evidenza, e i `[==testo==](ev:<id>)` già negli appunti
 * continuano a puntare dove puntavano.
 */
function postilla(vaultPath, courseId, id, testo) {
  ...
  const n = notaValida(testo);
  const voce = Object.assign({}, elenco[i]);
  if (n) voce.nota = n; else delete voce.nota;
  elenco[i] = voce;
  const scritto = salva(vaultPath, courseId, elenco);
  if (scritto.error) return { evidenza: null, evidenze: [], error: scritto.error };
  return { evidenza: scritto.evidenze[i], evidenze: scritto.evidenze, error: '' };
}
...
module.exports = {
  FILE, VISTA, CAMPI, TRATTI, TRATTO_DEFAULT, NOTA_MAX,
  dir, percorso, percorsoVista, corsoValido,
  identita, coloreValido, trattoValido, notaValida, normalizzaVoce,
  leggi, salva, aggiungi, rimuovi, colora, tratta, postilla, indice,
