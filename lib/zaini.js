...
function crea(vault, nome, quando) {
  ...
  /* ⚠️ Il controllo è sulle DUE radici: vedi la nota in testa al file. Il messaggio dice
     «una cartella» e non «uno zaino»: il nome può essere occupato da una cartella che questa
     app non mostra, e promettere uno zaino manderebbe l'utente a cercarlo in un elenco dove
     non c'è. Dice il fatto — sul disco quel nome è preso — e resta vero in tutti e due i casi. */
  if (idOccupato(vault, id)) return { error: 'esiste già una cartella con questo nome' };
  try {
...
function rinomina(vault, id, nome) {
  ...
  if (nuovo !== id && idOccupato(vault, nuovo)) {
    return { error: 'esiste già una cartella con questo nome' };
  }
