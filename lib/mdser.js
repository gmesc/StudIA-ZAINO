...
/** Nome cartella lezione: NN-slug */
function lessonFolder(order, title) { return nn(order) + '-' + slugify(title, 40); }
...
function chapter(ch) {
  const fm = [
    ...
    /* Le figure ritagliate dai documenti. Stanno nel frontmatter come `sources`
       e per la stessa ragione: la figura si VEDE grazie al rimando
       `![…](fig:NN#p=…&i=…)` scritto nel testo, ma il riquadro «Note e
       materiali» e il validatore hanno bisogno dell'elenco senza rileggere la
       prosa — e un elenco che si ricava due volte in due modi diverge. */
    ['figure', { rows: (ch.figure || []).map((f) => ({ pdf: String(f.pdf), p: Number(f.p || f.page) || 1, i: Number(f.i) || 1, label: f.label })) }],
    ['tags', { list: ch.tags || [] }],
...
/** _lezione.md — co: { id, title, tipo, area, materiali[], status, ordineCapitoli[], nota } */
function lezione(co) {
  const fm = [
    ['id', co.id],
    ['title', co.title],
 ...
 * le eccezioni: un materiale senza riga è materiale ufficiale della lezione.
...
/**
 * _corso.md — pr: { id, title, tipo, nLezioni, nMateriali, ordineLezioni[],
...
function corso(pr) {
  ...
  const fm = [
    ...
    ['title', pr.title],
    ['tipo', 'corso'],
    ['n_lezioni', Number(pr.nLezioni) || 0],
    ...
    ['ordine_lezioni', { list: pr.ordineLezioni || [] }]
  ];
...
/**
 * _profilo.md — p: { bisogni[], bisogniAltro, stileCorso, stileLezioni, stileCapitoli,
...
function profilo(p, isoDate) {
  const fm = [
    ...
    ['stile_corso', p.stileCorso || 'tematico'],
    ['stile_lezioni', p.stileLezioni || 'cornici'],
...
module.exports = {
  ...
  slugify, nn, chapterFilename, lessonFolder,
  chapter, lezione, corso, profilo,
