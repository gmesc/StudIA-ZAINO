...
/* ⚠️ Come in `lib/genera.js`: la somma sta in `lib/ai/provider.js` e non qui,
   perché la versione che elencava i campi perdeva `costoUsdDichiarato` — e con
   lui il costo vero di tutto ciò che passa da Claude Code. Torna sempre un
   oggetto, anche con la lista vuota: chi somma le schede parte da qui. */
function sommaUso(usi) {
  return provider.sommaUso(usi) || { inputTokens: 0, outputTokens: 0, chiamate: 0 };
}
...
async function analizzaTutti(vault, corso, materiali, opts) {
  ...
  async function operaio() {
    while (indice < lista.length) {
      ...
      // ⚠️ Si passa da `sommaUso`, non si sommano i campi a mano: erano tre
      //    addizioni che conoscevano tre campi su quattro, e il quarto era il costo.
      if (r.uso) esiti.uso = sommaUso([esiti.uso, r.uso]);
      if (r.errore) {
