 ...
 *
 * Le schede si scrivono nel vault (`Progetti/<p>/_schede/NN.json`): si pagano una
...
const mat = require('./materiali');   // Trascrizioni/Indici: prima dentro il progetto, poi nel vault
...
/** Cartella delle schede di un progetto (in `_lavorazione/`, col ripiego sul vecchio posto). */
const progettiLib = require('./progetti');
function cartella(vault, progetto) {
  const vecchia = path.join(vault, 'Progetti', progetto, '_schede');
  if (fs.existsSync(vecchia)) return vecchia;
  return path.join(progettiLib.lavorazione(vault, progetto), 'schede');
}
...
function leggi(vault, progetto, materiale) {
  try { return JSON.parse(fs.readFileSync(path.join(cartella(vault, progetto), nomeFile(materiale)), 'utf-8')); }
  catch (e) { return null; }
}

/** Scrive la scheda in modo atomico. */
function scrivi(vault, progetto, materiale, scheda) {
  const dir = cartella(vault, progetto);
...
/** Tutte le schede presenti per un progetto, indicizzate per numero di materiale. */
function tutte(vault, progetto, materiali) {
  const out = {};
  for (const m of materiali || []) {
    const s = leggi(vault, progetto, m);
...
async function analizzaMateriale(vault, progetto, materiale, ai) {
 ...
 */
async function analizzaTutti(vault, progetto, materiali, opts) {
  const o = opts || {};
  ...
  async function operaio() {
    while (indice < lista.length) {
      const mio = indice++;
      const m = lista[mio];
      if (!o.rifai && leggi(vault, progetto, m)) {
        esiti.saltate++;
      ...
      const r = await analizzaMateriale(vault, progetto, m, o.ai || {});
      ...
      scrivi(vault, progetto, m, r.scheda);
      esiti.fatte++;
