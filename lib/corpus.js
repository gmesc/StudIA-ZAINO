...
const mat = require('./materiali');   // i materiali possono stare dentro il progetto (o nel vault)
 ...
 * passeranno da un OCR che ne cambia il tipo. Finché resta qui fuori, un progetto
 ...
 * fornite dall'autore del corso hanno la terminologia giusta, dove Whisper può
 * sbagliare un nome proprio. I minutaggi restano quelli di Whisper: il testo
 * ufficiale non ne ha.
 */
function testoUfficiale(vault, stem, progetto) {
  const f = mat.trova(vault, 'Trascrizioni', stem + '.ufficiale.txt', progetto, !!progetto);
...
/**
 * Titolo di un PDF: il nome del file batte quasi sempre la copertina, che nei
 * corsi è un frontespizio uguale per tutti («Corso di Formazione Tutor DSA…»).
...
// Le parole vuote di due lingue: i corsi che l'utente porta dentro non sono
...
function digestMedia(vault, nome, progetto) {
  const stem = nome.replace(/\.[a-z0-9]+$/i, '');
  const tr = readJson(mat.trova(vault, 'Trascrizioni', stem + '.json', progetto, !!progetto));
  if (!tr) return nonElaborato(nome, 'video');
  const daWhisper = testoTrascrizione(tr);
  const ufficiale = testoUfficiale(vault, stem, progetto);
...
function digestHtml(vault, nome, progetto) {
  const stem = nome.replace(/\.[a-z0-9]+$/i, '');
  const idx = readJson(mat.trova(vault, 'Indice-HTML', stem + '.json', progetto, !!progetto));
...
function digestPdf(vault, nome, progetto) {
  const stem = nome.replace(/\.[a-z0-9]+$/i, '');
  const idx = readJson(mat.trova(vault, 'Indice-PDF', stem + '.json', progetto, !!progetto));
 ...
 *
 * Con `progetto` guarda SOLO i suoi (vedi `mat.cartelle(..., solo)`): senza
 * questo confine il digest di un progetto conteneva i materiali di tutti gli
 * altri, e chi legge il digest — le schede, la proposta dell'indice — li
 * trattava come propri.
 */
function materiali(vault, progetto) {
  const out = [];
  const visti = new Set();
  for (const sub of ['Media', 'Fonti']) {
    for (const { nome: f } of mat.elenca(vault, sub, progetto, !!progetto)) {
      if (visti.has(f)) continue; visti.add(f);
      const ext = path.extname(f).toLowerCase();
      if (ext === '.pdf') out.push(digestPdf(vault, f, progetto));
      else if (ext === '.html' || ext === '.htm') out.push(digestHtml(vault, f, progetto));
      else if (MEDIA_EXT.includes(ext)) out.push(digestMedia(vault, f, progetto));
    }
  }
  return out;
}
 ...
 * Con `progetto` è il corpus di QUEL progetto; senza, tutto il vault (uso
 ...
 */
function digest(vault, progetto) {
  const tutti = ordina(materiali(vault, progetto));
