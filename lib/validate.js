...
function validateCapitolo(c, ctx) {
  ...
  // 3. wikilink nella forma che il reader riconosce, e verso un corso che esiste.
  //    Il secondo controllo conta soprattutto quando un progetto cresce: un
  //    rimando a un corso mai scritto (o rinominato) manda l'allievo nel vuoto.
  ...
  for (const w of wl) {
    ...
    if (Array.isArray(ctx.corsi) && ctx.corsi.length && ctx.corsi.indexOf(target) < 0) {
      errors.push('wikilink [[' + target + ']] — nel progetto non c\'è un corso con questo nome');
    }
  }
