...
function eScansione(pagine) {
  ...
  /* L'ELENCO delle pagine senza testo, non solo il conto: è quello che un
     riconoscimento fermato a metà riprende in mano — si rileggono QUELLE, non
     tutto da capo. E una pagina che il testo ce l'ha già (nativa in mezzo alle
     foto, o già riconosciuta) non si ritocca: scriverle sopra un secondo layer
     raddoppierebbe ogni parola per la ricerca e per le evidenze. */
  const daRiconoscere = [];
  l.forEach((p, i) => {
    const car = String((p && p.text) || '').replace(/\s+/g, ' ').trim().length;
    if (car < CAR_SCANSIONE) daRiconoscere.push(Number.isFinite(+(p && p.page)) ? Math.trunc(+p.page) : i + 1);
  });
  return { scansione: l.length > 0 && daRiconoscere.length * 2 > l.length,
           pagineVuote: daRiconoscere.length, npagine: l.length, daRiconoscere };
}
