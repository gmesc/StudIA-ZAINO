 ...
 *     mentre l'utente aspetta di vedere un corso.
 *
 ...
 *     silenziosa che ha prodotto corsi senza PDF (vedi HANDOFF §7).
 */
...
function cartellaHub() {
  /* Le stesse variabili che guarda huggingface_hub, nello stesso ordine. Chi
     tiene i modelli su un disco esterno le imposta, e cercare a mano in
...
function modelloScaricato(nome) {
  const cartella = 'models--' + String(nome || MODELLO).replace(/\//g, '--');
  const hub = path.join(cartellaHub(), cartella);
  try {
    if (!fs.statSync(hub).isDirectory()) return { presente: false, dove: null, gb: 0 };
    return { presente: true, dove: hub, gb: pesoGb(path.join(hub, 'blobs')) };
  } catch (e) { return { presente: false, dove: null, gb: 0 }; }
}
...
function stato(userData) {
  ...
  const modello = modelloScaricato();
  return {
    installato,
    python: py,
...
function passoScaricaModello(userData) {
...
module.exports = {
  ...
  motivoPagina, selezionaPagine, durata, rigaStima, totaleStima,
  venvDir, python, importabile, versioni, cartellaHub, modelloScaricato, pesoGb, stato,
