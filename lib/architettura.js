'use strict';
/**
 * architettura — STADIO 1 multiagente: dalle schede all'architettura delle lezioni.
 *
...
const SISTEMA_SINTESI = [
  /* Il ruolo si chiamava «responsabile del percorso di studio»: da quando il
     Percorso è un oggetto dell'app — la traversata che sceglie una variante
     d'indice per ogni lezione — quel nome prometteva un lavoro che non è il suo.
     Attenzione: questo nome NON è più discriminante, perché lo condivide con
     propose.js e scaletta.js. Il modello finto smista su «ARCHITETTURA
     DEFINITIVA», che compare solo qui. */
  'Sei un instructional designer. Ricevi le schede dei materiali e tre analisi indipendenti:',
  'una tassonomia dei contenuti, una sequenza didattica con i prerequisiti, una mappa dei legami.',
  'Costruisci L\'ARCHITETTURA DEFINITIVA delle lezioni che compongono il corso.',
  'Regole non negoziabili:',
  '- ogni materiale deve stare in ESATTAMENTE una lezione: non inventarne, non ometterne;',
  '- una lezione è un blocco coerente di studio, non un contenitore di file vicini di numero;',
  '- l\'ordine delle lezioni rispetta i prerequisiti dichiarati dalla lente «sequenza»;',
  '- dove le tre analisi sono in disaccordo, decidi tu e scrivi la decisione in «decisioni», con il perché;',
  '- i materiali di sola consultazione restano un modulo-fonte a sé, non una lezione lineare;',
  '- «rimandi» elenca i collegamenti fra lezioni che il lettore dovrà poter seguire.'
].join('\n');
      ...
      lezioni: {
        ...
        items: {
          ...
          properties: {
            ...
            tipo: { type: 'string', enum: ['lezione', 'modulo-fonte'] },
            materiali: { type: 'array', items: { type: 'string' } }
          },
...
const SISTEMA_REVISIONE = [
  'Sei un revisore severo. Ti viene data un\'architettura di lezioni e le schede dei materiali da cui nasce.',
  'Cerca i difetti, in ordine di gravità:',
  '1. materiali mancanti o assegnati a due lezioni;',
  '2. lezioni incoerenti, che tengono insieme cose che non stanno insieme;',
  '3. ordine che viola un prerequisito;',
  '4. titoli che non dicono di cosa parla la lezione.',
