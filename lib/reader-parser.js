...
const END_MARK = 'function buildVaultProjects(';
...
function load(opts) {
  ...
  const sandbox = {
    ...
    /* Nel renderer le mappe NN→file si scelgono in base al progetto in
       lavorazione, e i due accessori stanno FUORI dal blocco estratto qui
       (che parte da `_unq`). Nel test non c'è un progetto: si risolve sulle
...
function loadNotes() {
  ...
  const sandbox = { console };
  vm.createContext(sandbox);
  vm.runInContext(code + '\nthis.__notes = { noteInChapter, noteStessoCorso, noteEtichetta, noteEtichettaAltrove, noteDove, noteGroups };', sandbox);
