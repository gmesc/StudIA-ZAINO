'use strict';
/**
 * scaletta — da una lezione approvata alle ALTERNATIVE di indice dei capitoli.
 *
 ...
 *    materiale della lezione finisce in ognuna di esse;
...
/**
 * Quante alternative chiedere, quando l'utente non lo dice: due sono poche per
 * scegliere, quattro sono troppe da leggere in fila su sette lezioni. È un valore
 * di partenza, non un limite: chi compone percorsi per otto personaggi ne vuole
 * di più, e ora lo può chiedere (`opts.nAlternative`).
 */
const N_ALTERNATIVE = 3;
/** Il numero riportato entro limiti sensati: sotto due non c'è scelta, sopra sei non si legge. */
function quante(n) {
  const q = Math.round(Number(n));
  return (q >= 2 && q <= 6) ? q : N_ALTERNATIVE;
}
...
/** Il prompt di sistema, col numero di alternative chiesto. */
function sistema(nAlternative) {
  return SISTEMA.replace('{N}', String(quante(nAlternative)));
}

const SISTEMA = [
  /* Il nome del ruolo è in comune con propose.js e architettura.js: a smistare
     questo prompt è «SCALETTE ALTERNATIVE», due righe sotto, che compare solo qui. */
  'Sei un instructional designer. Ricevi le schede dei materiali assegnati a UNA lezione',
  'e proponi {N} SCALETTE ALTERNATIVE dei suoi capitoli.',
  '',
  'Regole non negoziabili:',
  '- ogni alternativa copre TUTTO il materiale della lezione: cambiano ordine e raggruppamento, mai la copertura;',
                    ...
                    items: {
                      ...
                      properties: {
                        /* Il NUMERO, non il nome. Il campo si chiamava «materiale»
                           e basta, e il modello ci metteva il titolo del materiale:
...
/** I numeri dei materiali di una lezione, come stringhe a due cifre. */
function materialiDi(lezione) {
  return (lezione.materiali || []).map((m) => String(m.num || m.source || m).trim()).filter(Boolean);
}

/** Le schede dei soli materiali di questa lezione. */
function schedeDelLezione(lezione, schedeMappa) {
  const numeri = new Set(materialiDi(lezione));
  const fuori = {};
 ...
 *
 * Lasciato al modello, il numero oscilla fra 2 e 8 sulla stessa lezione, e
...
function vincoloCapitoli(n) {
  ...
  return [
    ...
    'sono pochi per il materiale della lezione, accorpa i temi vicini invece di lasciarne fuori qualcuno.'
  ].join('\n');
}

/** Il messaggio utente: schede della lezione + vincoli di forma + brief. */
function messaggio(lezione, schedeMappa, regoleForma, brief, nCapitoli, nAlternative) {
  return [
    '## Lezione «' + (lezione.title || lezione.folder) + '»',
    lezione.rationale ? 'Perché questi materiali stanno insieme: ' + lezione.rationale : '',
    '',
    '## Schede dei materiali della lezione',
    schede.testoPerPrompt(schedeDelLezione(lezione, schedeMappa)),
    '',
    regoleForma || '',
    brief ? '\n## Brief del corso\n' + brief : '',
    vincoloCapitoli(nCapitoli),
    '',
    'Proponi ' + quante(nAlternative) + ' scalette alternative. Rispondi solo con la struttura richiesta.'
  ].filter((r) => r !== '').join('\n');
}
 ...
 */
function numeroDiFonte(valore, lezione) {
  const grezzo = String(valore == null ? '' : valore).trim();
  const validi = new Set(materialiDi(lezione));
  ...
  if (!imp) return '';
  for (const m of (lezione.materiali || [])) {
...
function normalizza(alt, lezione) {
  const validi = new Set(materialiDi(lezione));
  const visti = new Set();
  const capitoli = (alt.capitoli || []).map((c) => {
    const fonti = (c.fonti || [])
      .map((f) => ({ materiale: numeroDiFonte(f.materiale, lezione), da: f.da, a: f.a }))
...
/**
 * Le alternative di scaletta per una lezione.
 * @returns {{alternative: Array, uso: Object|null, errore: string|null}}
 */
async function proponi(lezione, schedeMappa, ai, opts) {
  const o = opts || {};
  const r = await (ai.chiama || chiamaDefault)({
    fornitore: ai.fornitore, modello: ai.modello, apiKey: ai.apiKey, lingua: ai.lingua,
    sistema: sistema(o.nAlternative),
    utente: messaggio(lezione, schedeMappa, o.regoleForma, o.brief, o.nCapitoli, o.nAlternative),
    schema: schemaAlternative(),
    maxTokens: 12000
  });
  if (!r || !r.ok) return { alternative: [], uso: r && r.uso, errore: (r && r.errore) || 'nessuna risposta' };
  const alternative = togliDoppioni(((r.dati && r.dati.alternative) || []).map((a) => normalizza(a, lezione)))
...
module.exports = {
  N_ALTERNATIVE, quante, PRINCIPI, SISTEMA, sistema, schemaAlternative,
  materialiDi, schedeDelLezione, messaggio, vincoloCapitoli, impronta, numeroDiFonte, normalizza, togliDoppioni, proponi
};
