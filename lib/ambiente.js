...
/**
 * Dove Python 3 si installa davvero su Windows.
 *
 * ⚠️ Il problema è lo stesso del Mac ma con un protagonista diverso. Là il
 * rischio è trovare il 3.9 di sistema invece del 3.13 dell'utente; qui è
 * `WindowsApps\python3.exe`, che su una macchina senza Python **non è Python**:
 * è uno stub che apre il Microsoft Store. Non serve una regola apposta per
 * riconoscerlo — chiedergli `--version` non risponde con un numero, e il
 * controllo che c'è già lo scarta — ma va tenuto in fondo alla lista e va
 * saputo, perché è la ragione per cui su Windows «python c'è» e non funziona.
 *
 * Il launcher `py` è la via consigliata da python.org: non è un percorso ma un
 * comando, e si prova come tale (chi non ce l'ha, semplicemente non risponde).
 */
function candidatiPythonWindows() {
  const casa = os.homedir();
  const out = [];
  if (process.env.STUDIA_PYTHON) out.push(process.env.STUDIA_PYTHON);
  out.push('py');                                           // il launcher ufficiale
  /* Gli installer di python.org: una cartella per versione, sotto l'utente o
     per tutta la macchina. Si guarda che cosa c'è invece di indovinare i
     numeri di versione, che cambiano ogni anno. */
  const nidi = [
    path.join(casa, 'AppData', 'Local', 'Programs', 'Python'),
    'C:\\Program Files\\Python',
    'C:\\Program Files (x86)\\Python',
    'C:\\',
  ];
  for (const nido of nidi) {
    let dentro = [];
    try { dentro = fs.readdirSync(nido); } catch (e) {}
    for (const d of dentro) {
      if (!/^Python3?\d*/i.test(d)) continue;
      out.push(path.join(nido, d, 'python.exe'));
    }
  }
  out.push('python');                                       // quello che sta nel PATH, se c'è
  out.push(path.join(casa, 'AppData', 'Local', 'Microsoft', 'WindowsApps', 'python3.exe'));
  return out;
}
...
function candidatiPython() {
  if (process.platform === 'win32') return candidatiPythonWindows();
  const casa = os.homedir();
  const out = [];
  if (process.env.STUDIA_PYTHON) out.push(process.env.STUDIA_PYTHON);
 ...
 *  ⚠️ Su Windows non c'è nessuna shell di login da interrogare: la domanda
 *  equivalente la fa `where`, che dice quali eseguibili di quel nome stanno nel
 *  PATH — e ne può elencare più d'uno, che è esattamente il caso interessante. */
function chiediAllaShell() {
  if (process.platform === 'win32') {
    for (const nome of ['python', 'python3']) {
      try {
        const r = spawnSync('where', [nome], { encoding: 'utf-8', timeout: TIMEOUT });
        const righe = String((r && r.stdout) || '').trim().split(/\r?\n/).filter(Boolean);
        for (const p of righe) if (fs.existsSync(p)) return p;
      } catch (e) {}
    }
    return null;
  }
 ...
 */
function scegli(visti, esplicito, sistema) {
  ...
  /* ⚠️ «Quello di sistema» non è un percorso fisso: su un Mac è
     `/usr/bin/python3`, su Windows è il primo che sta nel PATH — spesso lo stub
     del Microsoft Store. Scritto a mano qui dentro, il confronto era vero solo
     su una delle due piattaforme e su Windows non avrebbe avvisato mai. */
  const dettoSistema = sistema || (process.platform === 'win32' ? null : '/usr/bin/python3');
  return {
    ...
    diversoDaSistema: !!dettoSistema && migliore.percorso !== dettoSistema &&
      visti.some((c) => c.percorso === dettoSistema)
  };
}
...
function pythonMigliore() {
  ...
  /* Su Windows «quello di sistema» va CHIESTO — è il primo del PATH, e cambia
     da macchina a macchina — mentre su un Mac è sempre lo stesso percorso. Si
     domanda una volta sola, e serve a decidere se vale la pena dire all'utente
     quale interprete si sta usando. */
  const sistema = process.platform === 'win32' ? chiediAllaShell() : '/usr/bin/python3';
  if (sistema && !gia.has(sistema)) {
    gia.add(sistema);
    const r = interroga(sistema);
    if (r) visti.push(r);
  }
  if (!visti.length) {
  ...
  return scegli(visti, process.env.STUDIA_PYTHON || null, sistema);
}
...
function consiglio(stato) {
  ...
  if (motore === 'nessuno') {
    avvisi.push({
      ...
      testo: 'Nessun motore AI disponibile: non si possono generare lezioni nuove. ' +
             'Le lezioni già scritte si leggono comunque, senza limitazioni.',
  ...
  const py = s.python || {};
  if (!py.trovato) {
    /* ⚠️ Il rimedio non è lo stesso ovunque: su un Mac Python arriva con gli
       strumenti da riga di comando, su Windows si scarica. E si dice
       python.org, non il Microsoft Store: lo stub dello Store si chiama
       `python3.exe`, sembra installato e non è Python (vedi
       `candidatiPythonWindows`). */
    avvisi.push({
      ...
      comando: process.platform === 'win32'
        ? 'installa Python 3 da python.org (spuntando «Add python.exe to PATH»)'
