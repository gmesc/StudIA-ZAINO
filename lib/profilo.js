...
/** Parsa il testo di _profilo.md o _corso.md. Ritorna sempre un oggetto. */
function parse(raw) {
  const out = { bisogni: [] };
  if (!raw) return out;
  const m = /^---\n([\s\S]*?)\n---/.exec(raw);
...
const TOKEN = {
  'attenzione-distraibilita': {
    proposta: 'unità da ≤10 minuti di lettura, al massimo ~15 capitoli per lezione, titoli che anticipano il contenuto',
...
function leve(p) {
  ...
  /* ⚠️ Il tetto vero è `maxItems` nello schema del capitolo (6 dal 12 agosto
     2026, prima 3): è quello che il modello riceve come structured output, e
     nessuna richiesta scritta qui può superarlo. Queste righe dicono quante
     puntarne DENTRO quel tetto — le due cose vanno alzate insieme, o si chiede
     un numero che la validazione rifiuta. Il ripasso (P3 di PIANO-BRAYNR) vive
     di carte: tre domande per capitolo erano poche per farci un mazzo. */
  if (p.quiz === 'frequenti') gen.push('5–6 quiz per capitolo');
  if (p.quiz === 'pochi') gen.push('2 quiz per capitolo');
  ...
  const stileP = { tematico: 'raggruppa i materiali per tema, non per data', cronologico: 'mantieni l\'ordine di erogazione dei materiali', problemi: 'organizza partendo da casi e problemi concreti' }[p.stile_corso];
  if (stileP) prop.push(stileP);
  const stileC = { cornici: 'apri ogni lezione con la mappa d\'insieme, poi scendi nel dettaglio', lineare: 'un passo alla volta, senza anticipazioni', spirale: 'torna sui concetti chiave a livelli via via più profondi' }[p.stile_lezioni];
...
function promptBlock(p, fase, indicazioniCorso) {
  ...
  if (fase === 'generazione') {
    const v = d.verbatim;
    if (v.comeImparo || v.cosaAffatica || v.altro || indicazioniCorso) {
      righe.push('');
      ...
      if (indicazioniCorso) righe.push('Indicazioni per questo materiale: ' + indicazioniCorso);
