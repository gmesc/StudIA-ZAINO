...
## Come si usa
- **App di studio**: apri `App/StudIA.html` nel browser (o in Electron). Dall'header usa **Importa** per caricare i corsi dalla cartella `Corsi/` (oppure trascinali dentro la finestra).
...
- `Corsi/` — i "database corso" in formato JSON, importabili nell'app.
...
## Corsi disponibili
- **Processi di apprendimento e Dislessia** — `Corsi/td74-03-04-processi-dislessia.json` (materiali 03–04, con link cliccabili alle pagine dei PDF)
- **I DSA in una prospettiva life-span** — `Corsi/td74-01-dsa-life-span.json` (videolezione 01)
- **La consulenza alla famiglia** — `Corsi/td74-02-consulenza-famiglia.json` (videolezione 02)
...
2. Indicizza: `python3 index_videos.py --transcripts Trascrizioni/ --course Corsi/<corso>.json --out Corsi/<corso>.json --video-ext .mp4`
