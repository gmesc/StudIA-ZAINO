...
# -*- coding: utf-8 -*-
"""
Indicizza le trascrizioni con timestamp e aggiunge a ogni capitolo di una lezione i
`videoRefs` = rimandi ai punti dei video dove si parla dell'argomento.

Uso:
  python3 index_videos.py --transcripts trascrizioni/ --lesson lezione.json --out lezione.json
Opzioni:
...
def index(transcripts, lesson, min_score=4, min_terms=2, max_refs=4, video_ext=None):
    # Scoping: se la lezione dichiara "videos": ["01","05",...] (prefissi numero modulo),
...
def main():
    ...
    index(lesson, lesson, min_score=a.min_score, min_terms=a.min_terms, max_refs=a.max_refs, video_ext=a.video_ext) if False else index(trs, lesson, a.min_score, a.min_terms, a.max_refs, a.video_ext)
    json.dump(lesson, open(a.out, "w", encoding="utf-8"), ensure_ascii=False, indent=2)
    n = sum(1 for c in lesson["chapters"] if c.get("videoRefs"))
    print("OK: %d/%d capitoli con videoRefs -> %s" % (n, len(lesson["chapters"]), a.out))
